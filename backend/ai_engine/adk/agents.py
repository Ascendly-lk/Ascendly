...
---------------------
Three Google ADK LlmAgent definitions — each responsible for one data source.
All agents use gemini-2.0-flash (GOOGLE_ADK_MODEL env var).

Agents:
  - industry_benchmark_agent  → World Bank + Alpha Vantage sector data
  - competitor_benchmark_agent → SerpAPI competitor search results
  - market_data_agent          → Yahoo Finance sector ETF performance

These are not run directly — they are composed by BenchmarkOrchestrator
in orchestrator.py using ADK's ParallelAgent pattern.
"""
import os
...
# ── Agent 2: Competitor Benchmark ─────────────────────────────────────────────

competitor_benchmark_agent = LlmAgent(
